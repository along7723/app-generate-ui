      ...
      <el-tree ref="myTree" :data="menuTree" node-key="id" show-checkbox default-expand-all draggable accordion
        ...
        <span slot-scope="{ node, data }">
          <template v-if="data.icon != null">
            ...
            <template v-else>
              <span>
                <svg-icon icon-class="data.icon"/>{{ node.label }}</span>
            </template>
          </template>
          <template v-else>
            <span><i :class="data.icon"/>{{ node.label }}</span>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      recursionMenu(arr, idArr) {
        arr.forEach(item => {
          idArr.push(item.id)
          if (item.hasChildren) {
            this.recursionMenu(item.children, idArr)
          }
        })
      },
      /**
      ...
      handleCheckClick(data, checkNode) {
        ...
        // 从第二级开始递归调用
        if (data.hasChildren) {
          this.recursionMenu(data.children, idArr)
        }
        ...
        if (addArr.length === 0) {
          this.$emit('menuRowUnChecked', idArr)
        } else {
          this.$emit('menuRowChecked', idArr)
        }
      }
    }
  }
