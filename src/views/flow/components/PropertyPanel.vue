  ...
  <div class="property-panel" ref="propertyPanel">
    <el-form :inline="true" :model="form" label-width="100px" size="small">
      ...
      <el-form-item label="节点类型">
        <el-input v-model="form.category" @input="categoryChange"></el-input>
      </el-form-item>
    ...
    methods: {
      handleModeler() {
        ...
        this.modeler.on("selection.changed", e => {
          ...
          console.log('节点选择element',element)
          this.element = element;
          console.log(this.element);
          if (!element) return;
          console.log('节点选择element.businessObject',element.businessObject)
          this.form = {
        ...
        this.modeler.on("element.changed", e => {
          const { element } = e;
          console.log('属性变化element',element)
          if (!element) return;
          //  新增节点需要更新回属性面板
      ...
      // 属性面板名称，更新回流程节点
      nameChange(name) {
        console.log('名称变化name',name)
        const modeling = this.modeler.get("modeling");
        modeling.updateLabel(this.element, name);
      },
      categoryChange(category) {
        console.log('名称变化category',category)
        const modeling = this.modeler.get("modeling");
        // modeling.updateLabel(this.element, category);
        modeling.updateProperties(this.element, { category: category });
      },
      // 属性面板颜色，更新回流程节点
      colorChange(color) {
        const modeling = this.modeler.get("modeling");
